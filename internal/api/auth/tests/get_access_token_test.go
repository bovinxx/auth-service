...
func TestGetAccessToken(t *testing.T) {
	...
	tests := []struct {
		name        string
		input       *auth_v1.GetAccessTokenRequest
		mockSetup   func()
	...
	}{
		{
			...
			mockSetup: func() {
				mockUserService.EXPECT().
					GetAccessToken(ctx, &models.Token{
						Token: oldToken,
					}).
		...
		{
			name: "failure - failed to get access token",
			input: &auth_v1.GetAccessTokenRequest{
				RefreshToken: oldToken,
			},
			mockSetup: func() {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := impl.GetAccessToken(ctx, tt.input)
			...
			if tt.wantErr {
				...
				assert.Equal(t, tt.wantToken.Token, resp.AccessToken)
			}
		})
	}
}
